...
export default function AIMealPlanScreen() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setError(e.message || 'Failed to generate meal plan. Make sure your API key is set.');
        } finally {
            setLoading(false);
        }
    };
                ...
                {!result ? (
                    <>
                        ...
                        <Card style={styles.targetCard}>
                            <Text style={styles.targetTitle}>Your Daily Targets</Text>
                            <View style={styles.targetRow}>
                                <View style={styles.targetItem}>
                                    <Text style={styles.targetValue}>{user?.daily_calorie_target || 2200}</Text>
                            ...
                            {dietProfile?.template && dietProfile.template !== 'standard' ? (
                                <Text style={styles.dietBadge}>🏷️ {dietProfile.template.replace('_', ' ')}</Text>
                            ) : null}
                        </Card>
                        ...
                        <View style={styles.chipGrid}>
                            {DAYS_OPTIONS.map((d) => (
                                <TouchableOpacity
                                    key={d}
                                    style={[styles.chip, numDays === d && styles.chipActive]}
                                    onPress={() => setNumDays(d)}
                                >
                                    <Text style={[styles.chipText, numDays === d && styles.chipTextActive]}>
                                        {d} day{d > 1 ? 's' : ''}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                            ...
                            <Card key={di} style={styles.dayCard}>
                                ...
                                >
                                    ...
                                    <View style={styles.dayStats}>
                                        <Text style={styles.dayCals}>{day.total_calories} kcal</Text>
                                        <Ionicons
                                ...
                                {expandedDay === di && (
                                    <View style={styles.dayContent}>
                                        {day.meals.map((meal, mi) => (
                                            <View key={mi} style={styles.mealItem}>
